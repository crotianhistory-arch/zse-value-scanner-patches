...
def _filing_from_resource(item: dict[str, Any], entities: dict[str, dict[str, Any]], source_url: str) -> ESEFFiling:
    ...
    xhtml_url = attrs.get("report_url")
    return ESEFFiling(
    ...
    *,
    ...
    response = client.get(
        ...
        headers={
            ...
            "User-Agent": "zse-value-scanner/0.4.0 (+bounded ESEF activity evidence discovery)",
        },
        timeout=timeout,
    )
    ...
    # API filter should already enforce this. Re-check locally so a malformed response
    # cannot silently attach evidence from a different entity.

...
def _candidate_from_record(record: dict[str, Any], *, query_name: str, query_country: str | None) -> GLEIFCandidate:
    ...
    # Conservative labels: only exact normalized legal name + country (when supplied)
    # is marked EXACT. Everything else remains review-oriented.
...
def search_legal_name(
    ...
    *,
    ...
    This function deliberately does not mutate the scanner database or warehouse.
    Candidate labels are review aids, not automatic entity merges.
    """
    ...
    headers = {
        ...
        "User-Agent": "zse-value-scanner/0.3.4 (+bounded GLEIF preflight)",
    }
    response = client.get(
